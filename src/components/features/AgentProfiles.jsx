...
function AgentCard({ profile, chores, colorScheme, birthdayProfiles = [], compact = false }) {
    ...
    return (
        <div className={`flex-1 ${compact ? 'min-w-[140px]' : 'min-w-[200px]'} border-2 bg-white dark:bg-black font-mono transition-all duration-500 ${color.border} ${color.glow} shadow-sm`}
            style={{ clipPath: compact 
                ? 'polygon(8px 0, 100% 0, 100% calc(100% - 8px), calc(100% - 8px) 100%, 0 100%, 0 8px)'
                : 'polygon(12px 0, 100% 0, 100% calc(100% - 12px), calc(100% - 12px) 100%, 0 100%, 0 12px)' 
            }}>

            {/* Header */}
            <div className={`${compact ? 'px-3 pt-2 pb-2' : 'px-4 pt-4 pb-3'} border-b ${color.border} border-opacity-30`}>
                <div className="flex justify-between items-start">
                    <div className="flex items-center gap-2">
                        <div className={`p-1 ${color.border} ${color.text} ${isOP ? 'animate-pulse' : ''}`}>
                            {isOP ? <Zap size={compact ? 10 : 14} /> : <Terminal size={compact ? 10 : 14} />}
                        </div>
                        <div>
                            <p className={`${compact ? 'text-[7px]' : 'text-[9px]'} font-bold uppercase tracking-[0.25em] ${color.text}`}>
                                {isBirthday ? '🎂' : isOP ? 'OP' : 'Agent'}
                            </p>
                            <p className={`${compact ? 'text-sm' : 'text-lg'} font-black uppercase tracking-widest text-slate-800 dark:text-white leading-none mt-0.5`}>
                                {profile.name}
                            </p>
                        </div>
                    </div>
                    <div className="text-right">
                        <span className={`${compact ? 'text-lg' : 'text-2xl'} font-black tracking-tighter ${color.text}`}>{currentXP}</span>
                        <span className="text-slate-400 text-[10px] font-bold ml-0.5">XP</span>
                    </div>
                </div>

                {/* XP Bar */}
                <div className={`${compact ? 'mt-1.5 h-1' : 'mt-3 h-2'} w-full bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800`}>
                    <div
            ...
            {/* Upcoming Chores - Hidden in compact mode */}
            {!compact && (
...
export function AgentProfiles({ profiles = [], chores = [], birthdayProfiles = [], compact = false }) {
    if (profiles.length === 0) return null;

    return (
        <div className={`flex flex-col h-full ${compact ? 'p-0' : 'p-6'}`}>
            {!compact && (
            ...
            <div className={`flex flex-row gap-4 flex-1 ${compact ? '' : 'overflow-x-auto pb-2'}`}>
