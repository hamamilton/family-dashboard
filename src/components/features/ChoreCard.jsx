...
export function ChoreCard({ chore, onToggle, onRotate, layout = 'original' }) {
                ...
                <div className={`${layout === 'list' ? 'flex-1 min-w-0 flex items-center justify-between' : ''}`}>
                    <div className={`${layout === 'list' ? 'flex-1 min-w-0 pr-4 flex items-center gap-4' : ''}`}>
                        <h3 className={`font-black leading-tight tracking-widest truncate ${
                            ...
                            'text-2xl mb-2'
                        } ${chore.is_completed ? 'line-through text-emerald-700 dark:text-emerald-700' : 'text-slate-800 dark:text-white drop-shadow-sm dark:drop-shadow-[0_0_5px_rgba(255,255,255,0.5)]'}`}>
                        ...
                        <div className={`flex items-center gap-2 ${layout === 'list' ? 'mt-0 flex-shrink-0' : 'mt-1'}`}>
                            ...
                            } ${chore.is_completed ? 'text-emerald-700 dark:text-emerald-900' : 'text-fuchsia-600 dark:text-fuchsia-500'}`}>
